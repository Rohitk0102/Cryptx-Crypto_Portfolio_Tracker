...
export class ForecastingService {
  ...
  private async generateNewForecast(symbol: string): Promise<ForecastResponse> {
    ...
    const [candlesticks, marketStats] = await Promise.all([
      this.marketDataClient.getCandlesticks(symbol, '1d', ninetyDaysAgo, now),
      this.marketDataClient.getMarketStats(symbol),
    ]);
